...
device.display = function(show=0, add=0)
  ...
  for i in user.tree
    ...
    for name in i.value.indexes.reverse;s=user.tree[i.key][name].s
      print user.tree[i.key]
      print device.display_tree.indexes
      c=user.tree[i.key][name].c
      f=user.tree[i.key][name].f
      res=res+b+("  -"+name.lower).color("purple purple black")+c10
    end for
  end for
