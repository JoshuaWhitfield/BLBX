//c0=char(0);c10=char(10);c33=char(33);tp=@typeof;cs=@clear_screen;ui=@user_input;gs=get_shell;hc=gs.host_computer;rtr=get_router

//string:
generate=function(len=100);set="qwertyuiopasdfghjklzxcvbnm"+"1234567890"+" !@#$%^&*()_+-={}|[]\:;'<>?,./"+"""";res="";rnd_ind=function();return floor((rnd * set.len-1) + 0);end function;for i in range(0,len-1);res=res+set[rnd_ind];end for;return res;end function
reveal=function(str, onCondition=0, elseShow="");if onCondition then return str;return elseShow;end function
//t_str=generate
//while tp(t_str.indexOf(" ")) != "number" ;t_str=generate();end while

string.has = function(subString)
    return typeof(self.indexOf(subString)) == "number"
end function

// --- arrays ---

list.has = function(subArray)
    return typeof(self.indexOf(subArray)) == "number"
end function

string.quote=function();return """"+self+"""";end function
string.elipsis=function(size=15);if self.len > size then return self[:(size-1)]+"...";return self;end function
string.copy=function();res="";for i in self;res=res+i;end for;self=res;return self;end function
string.search=function(value)
  if tp(self.lower.indexOf(value.lower)) == "number" then return true
  return false
end function

string.groupBy=function(size=10)
  res=[];count=1;inc_str=""
  for ch in self // ch=character
    if count > size then ;res.push(inc_str);inc_str="";count=1;end if
    inc_str=inc_str+ch
    count=count+1
  end for
  self=res
...
string.reverse=function()
  res=[]
  for i in self.values.reverse
    res.push(i)
  end for
  return res
end function

string.is_empty=function()
  alpha="qwertyuiopasdfghjklzxcvbnm"+"~`!@#$%^&*()_+-={}|[]\:;'<>?,./"+""""+"1234567890";res=1
  for i in self;if alpha.values.has(i.lower) then;res=0;break;end if;end for
  return res
end function
